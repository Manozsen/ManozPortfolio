    ...
    <div className="space-y-5 max-w-2xl">
        ...
        <div className="space-y-5">
          ...
          <div className="admin-card p-5 space-y-4">
            ...
            <div>
              ...
              <textarea value={hero.heroTitle} onChange={(e) => setHero((p) => ({ ...p, heroTitle: e.target.value }))} rows={3} className="input-dark resize-none" />
            </div>
            <div>
              <label className={lbl}>Subtitle</label>
              <textarea value={hero.heroSubtitle} onChange={(e) => setHero((p) => ({ ...p, heroSubtitle: e.target.value }))} rows={2} className="input-dark resize-none" />
            </div>
          </div>
          ...
          <div className="admin-card p-5 space-y-4">
            ...
            <Toggle value={hero.imageVisible} onChange={(v) => setHero((p) => ({ ...p, imageVisible: v }))} label="Show Image" sub="Toggle visibility" />
            <div>
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1">
                  <input ref={refs.heroImg} type="file" accept="image/*" className="hidden"
                    onChange={(e) => e.target.files?.[0] && upload(e.target.files[0], "heroImg", (url) => setHero((p) => ({ ...p, profileImageUrl: url })))} />
                  <button type="button" onClick={() => refs.heroImg.current?.click()} disabled={uploading.heroImg}
              ...
              {hero.profileImageUrl && (
                <button type="button" onClick={() => setHero((p) => ({ ...p, profileImageUrl: "" }))} className="text-xs text-red-400 text-center w-full">Remove</button>
              )}
            </div>
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {IMAGE_LAYOUTS.map((l) => (
                  <button key={l.value} type="button" onClick={() => setHero((p) => ({ ...p, imageLayout: l.value }))}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {IMAGE_SHAPES.map((s) => (
                  <button key={s.value} type="button" onClick={() => setHero((p) => ({ ...p, imageShape: s.value }))}
                    className={`px-3 py-2 rounded-xl border text-xs font-medium transition-colors text-center ${hero.imageShape === s.value ? "border-blue-600 bg-blue-600/20 text-blue-300" : "border-white/10 text-slate-400 hover:border-white/20"}`}>
                    {s.label}
                  </button>
                ))}
              </div>
            </div>
            <PillGroup options={IMAGE_SIZES} value={hero.imageSize} onChange={(v) => setHero((p) => ({ ...p, imageSize: v }))} label="Image Size" />
            <div>
              <label className={lbl}>Opacity — <span className="text-blue-400">{Math.round((hero.imageOpacity ?? 1) * 100)}%</span></label>
              <input type="range" min={0} max={1} step={0.05} value={hero.imageOpacity ?? 1}
          ...
          <div className="admin-card p-5 space-y-4">
            ...
            <PillGroup options={BG_TYPES} value={hero.backgroundType} onChange={(v) => setHero((p) => ({ ...p, backgroundType: v }))} label="Type" />
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {COLOR_PRESETS.map((preset) => (
                  <button key={preset.label} type="button"
                    ...
                    <div className="w-3.5 h-3.5 rounded-full" style={{ background: `linear-gradient(135deg, ${preset.primary}, ${preset.secondary})` }} />
              ...
              {([
                ...
                <div key={c.key}>
                  ...
                  <div className="flex items-center gap-1.5">
                    <input type="color" value={hero[c.key]} onChange={(e) => setHero((p) => ({ ...p, [c.key]: e.target.value }))} className="w-8 h-8 rounded-lg border border-white/10 bg-transparent cursor-pointer flex-shrink-0" />
                    <input type="text" value={hero[c.key]} onChange={(e) => setHero((p) => ({ ...p, [c.key]: e.target.value }))} className="input-dark flex-1 !py-1.5 font-mono text-xs" />
                  </div>
                </div>
              ))}
            </div>
              ...
              <div>
                ...
                {hero.backgroundImageUrl && (
                  <div className="relative h-16 rounded-xl overflow-hidden mb-2">
                    ...
                    <button type="button" onClick={() => setHero((p) => ({ ...p, backgroundImageUrl: "" }))} className="absolute top-1.5 right-1.5 w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                )}
                <input ref={refs.heroBg} type="file" accept="image/*" className="hidden"
                  onChange={(e) => e.target.files?.[0] && upload(e.target.files[0], "heroBg", (url) => setHero((p) => ({ ...p, backgroundImageUrl: url })))} />
                <button type="button" onClick={() => refs.heroBg.current?.click()} disabled={uploading.heroBg}
          ...
          {/* 3D controls */}
          <div className="admin-card p-5 space-y-4">
            <h2 className={sectionTitle}><Layers className="w-4 h-4 text-blue-400" />3D Scroll Transition</h2>
            <Toggle value={hero.enable3DTransition} onChange={(v) => setHero((p) => ({ ...p, enable3DTransition: v }))} label="Enable 3D Scroll" sub="Depth effect on scroll (desktop only)" />
            <Toggle value={hero.enable3D} onChange={(v) => setHero((p) => ({ ...p, enable3D: v }))} label="Enable 3D UI" sub="Global 3D perspective effects" />
            <Toggle value={hero.enableCardTilt} onChange={(v) => setHero((p) => ({ ...p, enableCardTilt: v }))} label="Card Tilt Effect" sub="Hover tilt on project cards" />
      ...
      {activeTab === "animate" && (
        <div className="space-y-5">
          ...
          <div className="admin-card p-5 space-y-4">
            <h2 className={sectionTitle}><Sparkles className="w-4 h-4 text-blue-400" />Background Animation</h2>
            <Toggle value={hero.enableAnimation !== false} onChange={(v) => setHero((p) => ({ ...p, enableAnimation: v }))} label="Enable Animation" sub="Animated gradient + floating orbs" />
            <PillGroup
              options={[
      ...
      {activeTab === "video" && (
        <div className="space-y-5">
          <div className="admin-card p-5 space-y-4">
            <h2 className={sectionTitle}><Video className="w-4 h-4 text-blue-400" />Background Video</h2>
            ...
            <Toggle value={hero.enableVideo} onChange={(v) => setHero((p) => ({ ...p, enableVideo: v }))} label="Enable Video" sub="Replaces gradient background" />
            <div>
              <label className={lbl}>Video URL (MP4)</label>
              <input value={hero.videoUrl} onChange={(e) => setHero((p) => ({ ...p, videoUrl: e.target.value }))} placeholder="https://res.cloudinary.com/.../video.mp4" className="input-dark" />
            </div>
            <div>
              ...
              {hero.fallbackImageUrl && (
                <div className="relative h-20 rounded-xl overflow-hidden mb-2">
                  ...
                  <button type="button" onClick={() => setHero((p) => ({ ...p, fallbackImageUrl: "" }))} className="absolute top-1.5 right-1.5 w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              )}
              <input ref={refs.fallback} type="file" accept="image/*" className="hidden"
                onChange={(e) => e.target.files?.[0] && upload(e.target.files[0], "fallback", (url) => setHero((p) => ({ ...p, fallbackImageUrl: url })))} />
              <button type="button" onClick={() => refs.fallback.current?.click()} disabled={uploading.fallback}
                className="flex items-center gap-2 bg-white/5 hover:bg-white/10 disabled:opacity-60 border border-white/10 text-white text-sm font-semibold px-4 py-2.5 rounded-xl w-full justify-center transition-colors">
      ...
      {activeTab === "section2" && (
        <div className="space-y-5">
          <div className="admin-card p-5 space-y-4">
            <h2 className={sectionTitle}><Layers className="w-4 h-4 text-blue-400" />Section Two</h2>
            <Toggle value={section2.enabled} onChange={(v) => setSection2((p) => ({ ...p, enabled: v }))} label="Enable Section" sub="Show this section after Hero" />
            <div>
              <label className={lbl}>Heading</label>
              <textarea value={section2.heading} onChange={(e) => setSection2((p) => ({ ...p, heading: e.target.value }))} rows={2} placeholder="Ready to grow your business online?" className="input-dark resize-none" />
            </div>
            <div>
              <label className={lbl}>Description</label>
              <textarea value={section2.description} onChange={(e) => setSection2((p) => ({ ...p, description: e.target.value }))} rows={3} className="input-dark resize-none" />
            </div>
            <div>
              <label className={lbl}>Button Text</label>
              <input value={section2.buttonText} onChange={(e) => setSection2((p) => ({ ...p, buttonText: e.target.value }))} placeholder="Get My Free Demo" className="input-dark" />
            </div>
            <div>
              <label className={lbl}>Button Link</label>
              <input value={section2.buttonLink} onChange={(e) => setSection2((p) => ({ ...p, buttonLink: e.target.value }))} placeholder="/request-demo" className="input-dark" />
            </div>
            <div>
              ...
              {section2.backgroundImageUrl && (
                <div className="relative h-20 rounded-xl overflow-hidden mb-2">
                  ...
                  <button type="button" onClick={() => setSection2((p) => ({ ...p, backgroundImageUrl: "" }))} className="absolute top-1.5 right-1.5 w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              )}
              <input ref={refs.sec2Bg} type="file" accept="image/*" className="hidden"
                onChange={(e) => e.target.files?.[0] && upload(e.target.files[0], "sec2Bg", (url) => setSection2((p) => ({ ...p, backgroundImageUrl: url })))} />
              <button type="button" onClick={() => refs.sec2Bg.current?.click()} disabled={uploading.sec2Bg}
                className="flex items-center gap-2 bg-white/5 hover:bg-white/10 disabled:opacity-60 border border-white/10 text-white text-sm font-semibold px-4 py-2.5 rounded-xl w-full justify-center transition-colors">
      ...
      {activeTab === "popup" && (
        <div className="space-y-5">
              ...
              <div className="bg-white rounded-xl border border-slate-200 overflow-hidden w-full max-w-xs shadow-lg">
                {popup.image && (
                  <div className="w-full bg-slate-100 flex items-center justify-center" style={{ maxHeight: "140px" }}>
                    <Image src={popup.image} alt="Popup" width={320} height={140} className="w-full h-auto object-contain" style={{ maxHeight: "140px" }} />
                  </div>
                )}
                <div className="p-4">
                  <h3 className="font-bold text-slate-900 text-sm font-display mb-1">{popup.title || "Popup Title"}</h3>
                  <p className="text-slate-500 text-xs mb-3 line-clamp-2">{popup.description || "Description"}</p>
                  <div className="flex gap-2">
                    <span className="flex-1 bg-blue-600 text-white text-xs font-semibold px-3 py-1.5 rounded-lg text-center">{popup.buttonText || "Button"}</span>
                    <span className="px-3 py-1.5 rounded-lg border border-slate-200 text-slate-400 text-xs">Later</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="admin-card p-5">
            <Toggle value={popup.enabled} onChange={(v) => setPopup((p) => ({ ...p, enabled: v }))} label="Enable Popup" sub="Shows on every page visit" />
          </div>
          <div className="admin-card p-5 space-y-4">
            <h2 className={sectionTitle}>Popup Content</h2>
            <div><label className={lbl}>Title</label><input value={popup.title} onChange={(e) => setPopup((p) => ({ ...p, title: e.target.value }))} placeholder="Special Offer" className="input-dark" /></div>
            <div><label className={lbl}>Description</label><textarea value={popup.description} onChange={(e) => setPopup((p) => ({ ...p, description: e.target.value }))} rows={3} className="input-dark resize-none" /></div>
            <div><label className={lbl}>Button Text</label><input value={popup.buttonText} onChange={(e) => setPopup((p) => ({ ...p, buttonText: e.target.value }))} className="input-dark" /></div>
            <div><label className={lbl}>Button Link</label><input value={popup.buttonLink} onChange={(e) => setPopup((p) => ({ ...p, buttonLink: e.target.value }))} className="input-dark" /></div>
            <div>
              <label className={lbl}>Popup Image (optional)</label>
              {popup.image && (
                <div className="relative mb-2 bg-slate-800 rounded-xl overflow-hidden flex items-center justify-center" style={{ maxHeight: "140px" }}>
                  <Image src={popup.image} alt="Popup" width={320} height={140} className="w-full h-auto object-contain" style={{ maxHeight: "140px" }} />
                  <button type="button" onClick={() => setPopup((p) => ({ ...p, image: "" }))} className="absolute top-2 right-2 w-6 h-6 bg-red-600 text-white rounded-full flex items-center justify-center"><X className="w-3 h-3" /></button>
                </div>
              )}
              <input ref={refs.popupImg} type="file" accept="image/*" className="hidden"
                onChange={(e) => e.target.files?.[0] && upload(e.target.files[0], "popupImg", (url) => setPopup((p) => ({ ...p, image: url })))} />
              <button type="button" onClick={() => refs.popupImg.current?.click()} disabled={uploading.popupImg}
                className="flex items-center gap-2 bg-white/5 hover:bg-white/10 disabled:opacity-60 border border-white/10 text-white text-sm font-semibold px-4 py-2.5 rounded-xl w-full justify-center transition-colors">
          ...
          <div className="admin-card overflow-hidden">
            ...
            <div className="p-4">
              <div className="bg-white rounded-xl p-4">
                ...
                <p className="text-blue-700 text-sm font-medium leading-tight mb-1 line-clamp-2">{seo.title || "Your Page Title"}</p>
                <p className="text-slate-500 text-xs line-clamp-2">{seo.description || "Your meta description will appear here."}</p>
              </div>
            </div>
          </div>
          <div className="admin-card p-5 space-y-4">
            <h2 className={sectionTitle}>SEO Settings</h2>
            <div><label className={lbl}>Meta Title</label><input value={seo.title} onChange={(e) => setSeo((p) => ({ ...p, title: e.target.value }))} className="input-dark" /><p className="text-xs text-slate-600 mt-1">{seo.title.length}/60</p></div>
            <div><label className={lbl}>Meta Description</label><textarea value={seo.description} onChange={(e) => setSeo((p) => ({ ...p, description: e.target.value }))} rows={3} className="input-dark resize-none" /><p className="text-xs text-slate-600 mt-1">{seo.description.length}/160</p></div>
            <div><label className={lbl}>Keywords</label><input value={seo.keywords} onChange={(e) => setSeo((p) => ({ ...p, keywords: e.target.value }))} className="input-dark" /></div>
            <div>
              <label className={lbl}>OG Image (1200×630px)</label>
              {seo.ogImage && (
                <div className="relative h-20 rounded-xl overflow-hidden mb-2">
                  <Image src={seo.ogImage} alt="OG" fill className="object-cover" sizes="400px" />
                  <button type="button" onClick={() => setSeo((p) => ({ ...p, ogImage: "" }))} className="absolute top-1.5 right-1.5 w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center"><X className="w-3 h-3" /></button>
                </div>
              )}
              <input ref={refs.ogImg} type="file" accept="image/*" className="hidden"
                onChange={(e) => e.target.files?.[0] && upload(e.target.files[0], "ogImg", (url) => setSeo((p) => ({ ...p, ogImage: url })))} />
              <button type="button" onClick={() => refs.ogImg.current?.click()} disabled={uploading.ogImg}
                className="flex items-center gap-2 bg-white/5 hover:bg-white/10 disabled:opacity-60 border border-white/10 text-white text-sm font-semibold px-4 py-2.5 rounded-xl w-full justify-center transition-colors">
      ...
      {activeTab === "general" && (
        <div className="space-y-5">
          <div className="admin-card p-5 space-y-4">
            ...
            <div><label className={lbl}>Main Headline</label><textarea value={general.homepageHeadline} onChange={(e) => setGeneral({ ...general, homepageHeadline: e.target.value })} rows={3} className="input-dark resize-none w-full" /></div>
            <div><label className={lbl}>Subtext</label><textarea value={general.homepageSubtext} onChange={(e) => setGeneral({ ...general, homepageSubtext: e.target.value })} rows={2} className="input-dark resize-none w-full" /></div>
          </div>
        </div>
      )}
