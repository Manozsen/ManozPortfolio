...
import type {
  User, DemoRequest, Demo, Project,
  AdminSettings, HeroSettings, PopupSettings,
  SeoSettings, SectionTwoSettings,
} from "@/types";
...
const DEFAULT_SECTION_TWO: SectionTwoSettings = {
  enabled: true,
  backgroundImageUrl: "",
  heading: "Ready to grow your business online?",
  description: "I build websites that turn visitors into customers. Fast, mobile-first, and designed to convert.",
  buttonText: "Get My Free Demo",
  buttonLink: "/request-demo",
};

export async function getSectionTwoSettings(): Promise<SectionTwoSettings> {
  const snap = await getDoc(doc(db, "settings", "sectionTwo"));
  if (!snap.exists()) return DEFAULT_SECTION_TWO;
  return { ...DEFAULT_SECTION_TWO, ...snap.data() } as SectionTwoSettings;
}

export async function updateSectionTwoSettings(data: Partial<SectionTwoSettings>): Promise<void> {
  await setDoc(doc(db, "settings", "sectionTwo"), { ...data, updatedAt: serverTimestamp() }, { merge: true });
}
...
const DEFAULT_POPUP: PopupSettings = {
  enabled: false,
  title: "Get a Free Website Demo",
  description: "See exactly what your website will look like before spending a rupee.",
  image: "",
