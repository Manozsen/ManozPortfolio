...
const LAYOUT_CONFIG = {
  ...
  "left-hero": {
    ...
    imageOrder: "order-1 md:order-2",
    textOrder: "order-2 md:order-1",
    maskImage: "radial-gradient(ellipse 88% 92% at 48% 38%, black 35%, rgba(0,0,0,0.6) 60%, transparent 80%)",
    showLeftFade: false,
    showRightFade: true,
    imageWidth: "w-[72vw] max-w-[320px] md:w-[44%] md:max-w-[520px]",
  },
  "center-blend": {
    ...
    imageOrder: "order-1",
    textOrder: "order-2",
    maskImage: "radial-gradient(ellipse 80% 85% at 50% 40%, black 30%, rgba(0,0,0,0.5) 60%, transparent 80%)",
    showLeftFade: false,
    showRightFade: false,
  ...
  "full-overlay": {
    ...
    imageOrder: "order-1",
    textOrder: "order-2",
    maskImage: "radial-gradient(ellipse 90% 95% at 50% 35%, black 25%, rgba(0,0,0,0.4) 55%, transparent 78%)",
    showLeftFade: false,
    showRightFade: false,
...
function buildBackground(hero: HeroSettings): React.CSSProperties {
  if (hero.backgroundType === "gradient") {
    return { background: `linear-gradient(135deg, ${hero.primaryColor} 0%, ${hero.secondaryColor} 100%)` };
  }
...
export default function HeroSection({ hero }: Props) {
  ...
  const isStretch =
    hero.imageLayout === "right-hero" || hero.imageLayout === "left-hero";
    ...
    <section className="relative min-h-screen flex items-center overflow-hidden" style={bgStyle}>
      ...
      {hero.backgroundType === "image" && hero.backgroundImageUrl && (
        <>
          <div className="absolute inset-0 z-0">
            <Image src={hero.backgroundImageUrl} alt="Hero background" fill className="object-cover" sizes="100vw" priority />
          </div>
          <div className="absolute inset-0 z-0" style={{ backgroundColor: hero.primaryColor, opacity: 0.75 }} />
        </>
      )}
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        <div
          ...
          style={{ background: `radial-gradient(circle, ${hero.primaryColor}44 0%, transparent 70%)`, filter: "blur(40px)" }}
        />
        <div
          className="absolute top-1/2 right-[5%] -translate-y-1/2 w-[650px] h-[650px] rounded-full"
          style={{ background: `radial-gradient(circle, ${hero.secondaryColor}55 0%, transparent 65%)`, filter: "blur(60px)" }}
        />
        <div
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-16">
        <div className={`flex ${layout.containerClass} items-center justify-between gap-0 min-h-screen md:min-h-0 md:py-0 pt-24 pb-12`}>

          {/* Text */}
          <div className={`flex-1 max-w-2xl text-center md:text-left z-10 ${layout.textOrder}`}>
            ...
            <div className="inline-flex items-center gap-2 mb-6">
              ...
              <span className="text-xs font-medium tracking-wide opacity-70" style={textStyle}>Available for new projects</span>
            </div>

            <p className="text-sm font-semibold mb-3 tracking-widest uppercase font-display animate-fade-up" style={{ color: "#60a5fa" }}>
              {hero.heroTagline}
            </p>

            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-[1.08] mb-6 font-display animate-fade-up delay-100" style={textStyle}>
              {before}
              {after && (
            ...
            <p className="text-base md:text-lg leading-relaxed mb-8 max-w-md mx-auto md:mx-0 animate-fade-up delay-200 opacity-75" style={textStyle}>
              {hero.heroSubtitle}
            </p>
            ...
            <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-10 animate-fade-up delay-300">
              {["Online Creators", "Local Businesses", "Instagram Sellers"].map((tag) => (
                <span key={tag} className="text-xs font-medium px-3 py-1.5 rounded-full border border-white/10 bg-white/[0.06]" style={textStyle}>
                  {tag}
                </span>
              ))}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center md:justify-start animate-fade-up delay-400">
              ...
              <a
                href={`https://wa.me/916296622391?text=${encodeURIComponent("Hi Manoz! I saw your portfolio and I'd like to discuss my project.")}`}
                target="_blank"
            ...
            <p className="text-xs mt-6 animate-fade-up delay-500 opacity-40" style={textStyle}>
          ...
          {/* Blended portrait */}
          <div
            className={`relative flex-shrink-0 flex items-end justify-center ${layout.imageWidth} ${layout.imageOrder}`}
            style={isStretch ? { alignSelf: "stretch" } : {}}
          >
            <div className="absolute inset-0 pointer-events-none" style={{ background: `radial-gradient(ellipse 75% 85% at 52% 45%, ${hero.secondaryColor}55 0%, transparent 68%)` }} />

            <div
              className="relative w-full"
              style={isStretch ? { aspectRatio: "3/4", maxHeight: "85vh" } : { aspectRatio: "3/4", maxHeight: "70vh" }}
            >
              <Image
                src={imageSource}
